...
public class App implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        if (document.current.state.reported.temperature.equals(
                ...
                document.current.state.reported.body_temperature.equals(
                        document.previous.state.reported.body_temperature) &&
                document.current.state.reported.pulse.equals(
                        document.previous.state.reported.pulse) &&
                document.current.state.reported.PAN.equals(
                        document.previous.state.reported.PAN)) {
            return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item()
                        ...
                        .withString("body_temperature", document.current.state.reported.body_temperature)
                        .withString("pulse", document.current.state.reported.pulse)
                        .withString("PAN", document.current.state.reported.PAN)
